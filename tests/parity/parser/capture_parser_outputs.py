...
def run_parser(family: str, case: dict) -> dict:
    ...
    try:
        if "chunks" in case:
            parse_stream = getattr(wrapper, "parse_stream", None)
            if parse_stream is None:
                return {
                    "calls": None,
                    "normal_text": None,
                    "error": f"UNAVAILABLE: {IMPL} wrapper has no stream parser",
                }
            got = parse_stream(family, case["chunks"], case.get("tools"))
        elif "model_text" in case:
            got = wrapper.parse(family, case["model_text"], case.get("tools"))
        else:
            return {
                "calls": None,
                "normal_text": None,
                "error": "PYTHON_EXC: fixture case has neither `model_text` nor `chunks`",
            }
        return {
