...
that backs the multi-worker dispatch checks for DIS-2098 — without
needing a GPU or a real vLLM worker. We monkeypatch ``requests.get`` to
return canned ``/metrics`` responses that mimic the
