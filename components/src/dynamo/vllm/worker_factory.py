...
class WorkerFactory:
    ...
    async def _create_embedding_worker(
        ...
        - InstrumentedScheduler: hard-codes ``pooling_params=None`` (see
          ...
          lower than external HTTP load testing (the DIS-2095 methodology,
          which is what every other embedding-serving stack uses anyway).
          The single remaining wedge is planner integration: if/when the
          Dynamo planner needs in-process embedding capability curves to
          auto-scale embedding fleets, add ``--benchmark-mode embed`` at
          that point together with the planner's embedding-capability model.
          See DIS-2092 appendix for the full reasoning.
