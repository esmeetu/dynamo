...
struct RequestControlMessage {
    ...
    /// For bidirectional dispatch (`request_type == ManyIn`): connection info the worker
    /// dials back to in order to receive subsequent request frames. `None` for the unary
    /// path, which is the wire-compatible default.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    request_stream_connection_info: Option<ConnectionInfo>,
}
