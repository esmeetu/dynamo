...
///
/// Dynamo emits the `model` label from multiple code paths (HTTP frontend
/// request lifecycle, frontend model-config gauges, backend worker metrics
/// arriving via `metrics_labels`). Without a shared normalization, the same
/// logical model can produce distinct Prometheus series for different input
/// casings (e.g. `Qwen/Qwen3-0.6B` vs `qwen/qwen3-0.6b`), splitting data
/// across dashboards that filter by `model="$model"`. Every metric site
/// that records the `model` label should run its value through this helper
/// before passing it to `with_label_values`.
