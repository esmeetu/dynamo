...
// path matches the loader merge path (same applyGMSClientSpec helper).
func TestEnsureGMSCheckpointJobSidecars_SaverOverride(t *testing.T) {
	...
	spec := &nvidiacomv1alpha1.GMSCheckpointSpec{
		...
		Loader: &nvidiacomv1alpha1.GMSClientSpec{Image: "should-not-be-used:latest"},
	}
...
// TestApplyGMSClientSpec_NilIsNoop directly covers the helper for paranoia.
func TestApplyGMSClientSpec_NilIsNoop(t *testing.T) {
	...
	out := applyGMSClientSpec(base, nil)
	assert.Equal(t, base, out, "nil spec is byte-identical no-op")
}

// TestApplyGMSClientSpec_EmptySpecIsNoop covers the "checkpoint.loader: {}"
...
func TestApplyGMSClientSpec_EmptySpecIsNoop(t *testing.T) {
	base := corev1.Container{
		Name:    GMSLoaderContainer,
		Image:   "base:1",
		Command: []string{"python3", "-m", gmsCheckpointLoaderModule},
	}
	out := applyGMSClientSpec(base, &nvidiacomv1alpha1.GMSClientSpec{})
...
// TestApplyGMSClientSpec_EmptyEnvFromSecretIgnored asserts an explicitly empty
...
func TestApplyGMSClientSpec_EmptyEnvFromSecretIgnored(t *testing.T) {
